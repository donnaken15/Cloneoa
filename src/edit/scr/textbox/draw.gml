// READONLY
// very slow despite usage optimization
...

{
    ...
    if (textbox_focus=i) {
        ...
        keys[0]=vk_enter keys[1]=vk_backspace keys[2]=vk_delete keys[3]=ord("V") keys[4]=vk_right keys[5]=vk_left keys[6]=vk_up keys[7]=vk_down keys[8]=vk_home keys[9]=vk_end
        for (k=0 k<8 k+=1) {
            key_press[keys[k]]=0
        ...
        if (mouse_check_button_pressed(mb_left) && !keyboard_check(vk_shift) && mouse_y>18) textbox_focus=-1
